...
function updateDigitDisplay(position, change) {
    const digitContainer = document.querySelector(`.digit-control[data-position="${position}"]`);
...
function updateTimerDisplay() {
    ...
    const hourElem = document.querySelector(`.digit-control[data-position="h"] .time-digit`);
    if (hourElem) {
    ...
    for (const [pos, val] of Object.entries(minuteMap)) {
        const elem = document.querySelector(`.digit-control[data-position="${pos}"] .time-digit`);
        if (elem) {
...
async function resetStopwatch() {
    ...
    const confirmed = await notificationService.confirm('stopwatch.reset.confirm');
    if (!confirmed) return;
...
function resetAllInputs() {
    ...
    if (minutesInput) minutesInput.value = '';
    if (customTimerInput) {
        customTimerInput.value = Math.floor(AppState.get('timer.workDuration') / 60);
    }
}
...
function updateUIAfterSave() {
    updateDOMIfExists('time-left', `${Math.floor(AppState.get('timer.workDuration') / 60)}:00`);
