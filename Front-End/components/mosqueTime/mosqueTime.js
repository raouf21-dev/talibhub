...
export class MosqueTimeManager {
  ...
  getDefaultContainer() {
    if (this.isWelcomePage) {
      return document.getElementById("welcomepage-mosque-container");
    } else {
      return (
  ...
  async initialize() {
    try {
      ...
      console.log("MosqueTimeManager: Options:", {
        isWelcomePage: this.isWelcomePage,
      });

      // Vérifier le container avec plus de détails
      console.log("MosqueTimeManager: Checking container...");
      const mosquetimeElement = document.getElementById("mosquetime");
      const mosquetimeContent = document.querySelector(
        "#mosquetime .mosquetime-content"
      );
      console.log("MosqueTimeManager: Found elements:", {
        mosquetimeElement: !!mosquetimeElement,
        mosquetimeContent: !!mosquetimeContent,
        containerAssigned: !!this.container,
      });

      if (!this.container) {
        console.error("MosqueTimeManager: Container not found");
        console.error("MosqueTimeManager: Available elements in DOM:", {
          mosquetime: !!document.getElementById("mosquetime"),
          content: !!document.querySelector(".mosquetime-content"),
      ...
      console.log(
        "MosqueTimeManager: Container found:",
      ...
      console.log("MosqueTimeManager: Initialization completed successfully");
    } catch (error) {
      console.error("MosqueTimeManager: Initialization failed:", error);
      if (error.message && error.message.includes("token")) {
        console.log(
          "MosqueTimeManager: Token error detected, redirecting to login"
        );
        window.location.href = "/login";
      } else {
        // Pour les autres erreurs, continuons mais informons l'utilisateur
        console.error(
          "MosqueTimeManager: Non-fatal error during initialization:",
          error
        );
        if (typeof notificationService !== "undefined") {
          notificationService.show("mosquetime.init.error", "error");
        }
      }
    }
  }

  // Nouvelle méthode pour ajouter les styles personnalisés (mode authentifié)
  addCustomStyles() {
  ...
  // Méthode pour charger la dernière ville sélectionnée (mode authentifié)
  ...
  setupEventListeners() {
    ...
    let sortButtonIds, citySelectId, locationButtonId, searchButtonId;

    if (this.isWelcomePage) {
      sortButtonIds = {
        asc: "welcome-mosquetime-sort-asc",
        desc: "welcome-mosquetime-sort-desc",
        nearest: "welcome-mosquetime-sort-nearest",
      };
      citySelectId = "welcome-mosquetime-city";
      locationButtonId = "welcome-mosquetime-location";
      searchButtonId = "welcome-mosquetime-search";
    } else {
      sortButtonIds = {
        asc: "mosquetime-sort-asc",
        desc: "mosquetime-sort-desc",
        nearest: "mosquetime-sort-nearest",
      };
      citySelectId = "mosquetime-location-select";
      locationButtonId = "mosquetime-use-location";
      searchButtonId = "mosquetime-search";
    }

    // Gestion du tri
    const sortButtons = {
      asc: document.getElementById(sortButtonIds.asc),
      desc: document.getElementById(sortButtonIds.desc),
      nearest: document.getElementById(sortButtonIds.nearest),
    };
    ...
    console.log(
      "MosqueTimeManager: City select found:",
      !!citySelect,
      citySelectId
    );
  ...
  // Méthode pour gérer la demande de localisation
  ...
  // Surcharge de la méthode de cache - utilise mosqueTimesStorageService
  getCacheKey(key) {
    if (this.isWelcomePage) {
      // Pour la welcome page, ajouter le préfixe public
      return `public_${key}`;
    } else {
      // Pour le mode authentifié, utiliser directement le service
      return key;
    }
  }
  ...
  async loadCities() {
    try {
      console.log("[DEBUG] MosqueTimeManager: Starting loadCities");

      // Utiliser api au lieu de fetch avec le bon chemin
      const cities = await api.get("/mosque-times/cities/search");
      console.log("MosqueTimeManager: Cities loaded:", cities);

      // Determine the correct city select ID based on the mode
      let citySelectId;
      if (this.isWelcomePage) {
        citySelectId = "welcome-mosquetime-city";
      } else {
      ...
      if (citySelect) {
        ...
        const lastCity =
          mosqueTimesStorageService.getLastSelectedCity() || cities[0];
        if (lastCity) {
          console.log(
            "MosqueTimeManager: Loading last selected city:",
            lastCity
          );
        ...
        console.error(
          `MosqueTimeManager: City select not found: ${citySelectId}`
        );
      }
    } catch (error) {
      console.error("MosqueTimeManager: Error loading cities:", error);
    }
  }

  // Méthode pour changer d'onglet selon le mode
  switchTab(tabName) {
    console.log("MosqueTimeManager: Switching to tab:", tabName);

    let contentElements;

    if (this.isWelcomePage) {
      // IDs pour la page de bienvenue
      contentElements = {
      ...
      // IDs pour la page authentifiée
      contentElements = {
        singleContent: document.getElementById("mosquetime-single-mosque"),
        allContent: document.getElementById("mosquetime-all-mosques"),
      };
    }

    console.log("MosqueTimeManager: Content elements:", contentElements);

    // Vérifier que les éléments existent avant de les manipuler
    if (!contentElements.singleContent || !contentElements.allContent) {
      console.error(
        "MosqueTimeManager: Required DOM elements not found for tab switching"
      );
      console.error("MosqueTimeManager: Available elements:", {
        singleContent: !!contentElements.singleContent,
        allContent: !!contentElements.allContent,
        isWelcomePage: this.isWelcomePage,
      });
      return;
    }
  ...
  // Méthode pour peupler le sélecteur de mosquées selon le mode
  populateMosqueSelect(mosques) {
    console.log("MosqueTimeManager: Populating mosque select with:", mosques);

    let selectId;
    if (this.isWelcomePage) {
      selectId = "welcome-mosquetime-mosque-select";
    } else {
      selectId = "mosquetime-mosque-select";
    }

    const select = document.getElementById(selectId);
    if (!select) {
      console.error(
        `MosqueTimeManager: Mosque select element not found: ${selectId}`
      );
      return;
    }
    ...
    select.addEventListener("change", () => {
      console.log("MosqueTimeManager: Mosque select changed");
      this.updateSingleMosqueTimes();
    });
  }

  // Méthode pour afficher toutes les mosquées selon le mode
  async displayAllMosques() {
    console.log("[DEBUG] MosqueTimeManager: Displaying all mosques");

    let gridId;
    if (this.isWelcomePage) {
      gridId = "welcome-mosquetime-grid";
    } else {
      gridId = "mosquetime-all-mosques-list";
    }

    const grid = document.getElementById(gridId);
    if (!grid) {
      console.error(
        `[DEBUG] MosqueTimeManager: Grid element not found: ${gridId}`
      );
      return;
    }
  ...
  // Méthode pour l'affichage d'une seule mosquée selon le mode
  async updateSingleMosqueTimes() {
    console.log("MosqueTimeManager: Updating single mosque times");

    let mosqueSelectId, contentDivId;

    if (this.isWelcomePage) {
      mosqueSelectId = "welcome-mosquetime-mosque-select";
      contentDivId = "welcome-mosquetime-single-content";
    } else {
      mosqueSelectId = "mosquetime-mosque-select";
      contentDivId = "mosquetime-single-content";
    }

    const mosqueSelect = document.getElementById(mosqueSelectId);
    const contentDiv = document.getElementById(contentDivId);

    if (!mosqueSelect || !contentDiv) {
      console.error("MosqueTimeManager: Required elements not found:", {
        mosqueSelectId,
        contentDivId,
        mosqueSelectFound: !!mosqueSelect,
        contentDivFound: !!contentDiv,
      });
      return;
    }

    console.log("MosqueTimeManager: Selected mosque ID:", mosqueSelect.value);
    console.log("MosqueTimeManager: Current mosques:", this.currentMosques);

    const selectedMosque = this.currentMosques.find(
      (m) => m.id === parseInt(mosqueSelect.value)
    );

    console.log("MosqueTimeManager: Selected mosque data:", selectedMosque);
  ...
  async handleCitySelection(cityName) {
    try {
      ...
      console.log(
        "[DEBUG] MosqueTimeManager: Données en cache pour",
        cityName,
      ...
      if (cachedData) {
        console.log(
          "[DEBUG] MosqueTimeManager: Utilisation des données en cache pour",
          cityName,
          cachedData
        );
...
// --- Fonction d'export pour compatibilité avec l'API existante ---

/**
 * Point d'entrée principal pour initialiser le module Mosque Time
 * Fonction d'export pour maintenir la compatibilité avec l'ancienne API
 */
export async function initializeMosqueTime() {
  console.log(
    "initializeMosqueTime: Initializing MosqueTimeManager for authenticated mode"
  );

  try {
    // Vérifier que la page mosquetime est visible
    const mosquetimeSection = document.getElementById("mosquetime");
    if (!mosquetimeSection) {
      console.error(
        "initializeMosqueTime: Section #mosquetime not found in DOM"
      );
      return null;
    }

    console.log(
      "initializeMosqueTime: mosquetime section found, proceeding with initialization"
    );
    ...
    // Exposer l'instance globalement si nécessaire pour le debugging
    window.mosqueTimeManager = mosqueTimeManager;

    console.log("initializeMosqueTime: Initialization completed successfully");
    return mosqueTimeManager;
  } catch (error) {
    console.error("initializeMosqueTime: Failed to initialize:", error);

    // Notifier l'utilisateur du problème
    if (typeof notificationService !== "undefined") {
      notificationService.show("mosquetime.init.failed", "error");
    }
