...
const CacheService = {
  ...
  setItem(key, data, expirationTimestamp) {
    ...
    try {
      ...
      console.error(
        `Erreur lors de la sauvegarde dans le cache pour la clé ${key}:`,
        error
      );
    }
  },
  ...
  getItem(key) {
    ...
    try {
      ...
      console.error(
        `Erreur lors du parsing du cache pour la clé ${key}:`,
        error
      );
      localStorage.removeItem(key);
      return null;
...
// Attacher getMidnightTimestamp à l'objet CacheService
