...
class MosqueTimesController {
    ...
    async manualScrape(req, res) {
        try {
            ...
            for (const mosque of mosques) {
                ...
                if (data) {
                    await mosqueTimesModel.savePrayerTimes(mosque.id, data.date, data.times);
                }
            }
        ...
        try {
            ...
            for (const city of cities) {
                ...
                for (const item of data) {
                    await mosqueTimesModel.savePrayerTimes(item.mosqueId, item.date, item.times);
                }
            }
    ...
    async searchMosques(req, res) {
        try {
            ...
            const mosques = await mosqueTimesModel.searchMosques(parseFloat(lat), parseFloat(lon));
            res.json(mosques);
        } catch (error) {
    ...
    async addMosque(req, res) {
        try {
            ...
            const mosqueId = await mosqueTimesModel.addMosque(name, address, parseFloat(latitude), parseFloat(longitude));
    ...
    async getPrayerTimesForCityAndDate(req, res) {
        try {
            ...
            return res.json({ 
                ...
                message: formattedPrayerTimes.length === 0 ? "Aucun horaire trouvé pour cette date" : undefined
            });
...
module.exports = {
    ...
    getPrayerTimesForCityAndDate: controller.getPrayerTimesForCityAndDate.bind(controller),
