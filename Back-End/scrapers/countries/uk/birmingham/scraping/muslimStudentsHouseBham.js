...
const handleCloudflare = async (page) => {
  try {
    ...
    if (hasChallenge) {
      console.log(
        "Protection Cloudflare détectée, tentative de contournement..."
      );
 ...
 * - Interception ressources (images, CSS, fonts)
 */
const scrapeMSHUK = createOptimizedScraper(async (page) => {
  ...
  await humanBehavior.moveMouseRandomly(
    page,
    ".section-prayer-horizontal-times"
  );
  ...
  const rawData = await page.evaluate(() => {
    ...
    const prayerItems = document.querySelectorAll(
      ".section-prayer-horizontal-times-item"
    );
    const validPrayers = new Set([
      "fajr",
      "zuhr",
      "dhuhr",
      "asr",
