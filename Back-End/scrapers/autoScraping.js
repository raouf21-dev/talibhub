...
function logError(message, error) {
  console.error(`[AUTOSCRAPING] ${new Date().toISOString()} - ERROR: ${message}`, error);
}
...
async function checkIfPrayerTimesExist() {
  ...
  try {
    ...
    logInfo(`Résultat de la vérification: ${dataExists ? 'Les données existent' : 'Aucune donnée trouvée'}`);
    return dataExists;
  } catch (error) {
...
function scheduleRandomScraping() {
  try {
    ...
    logInfo(`Planification du scraping pour minuit ${randomMinutes}m:${randomSeconds}s (Fuseau horaire serveur: ${Intl.DateTimeFormat().resolvedOptions().timeZone})`);
    logInfo(`Expression cron utilisée: ${cronExpression}`);
    
    // Schedule the scraping task
    const task = cron.schedule(cronExpression, async () => {
      logInfo(`🔄 CRON TRIGGERED: Démarrage de la vérification à ${new Date().toLocaleTimeString()}`);
      ...
      try {
        ...
        if (timesExist) {
          logInfo(`ℹ️ Les horaires de prière pour aujourd'hui existent déjà dans la base de données. Scraping ignoré.`);
        } else {
          logInfo(`🔍 Aucun horaire trouvé pour aujourd'hui. Démarrage du scraping...`);
          
          try {
            // Exécuter le scraping de toutes les villes
            // Importer correctement le contrôleur en utilisant la méthode de fond
            const mosqueTimesController = require('../controllers/mosqueTimesController');
            logInfo("Contrôleur importé avec succès, lancement du scraping...");
            
            // Utiliser directement la méthode background pour éviter les problèmes de contexte
            await mosqueTimesController.scrapeAllCitiesBackground();
            
            logInfo(`✅ Scraping automatique terminé avec succès à ${new Date().toLocaleTimeString()}`);
          } catch (scrapingError) {
        ...
        logInfo("Erreur rencontrée, replanification pour demain malgré l'erreur");
        task.stop();
        scheduleRandomScraping();
      }
    });
    ...
    logError("Erreur lors de la configuration du scraping automatique:", setupError);
    // En cas d'erreur critique, on réessaie dans 1 heure
    logInfo("Tentative de replanification dans 1 heure suite à une erreur critique");
...
logInfo(`Fuseau horaire du serveur: ${Intl.DateTimeFormat().resolvedOptions().timeZone || 'Non détecté'}`);
...
setTimeout(async () => {
  ...
  try {
    ...
    logInfo(`Test de vérification des données terminé avec résultat: ${result}`);
  } catch (testError) {
