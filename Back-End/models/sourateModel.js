...
const saveKnownSourates = async (userId, sourates) => {
  ...
  try {
    ...
    await client.query("DELETE FROM known_sourates WHERE user_id = $1", [
      userId,
    ]);
    const insertQuery = `
...
const recordRecitation = async (userId, firstSourate, secondSourate) => {
  ...
  try {
    ...
    const insertRecitationsQuery = `
        ...
        `;
    await client.query(insertRecitationsQuery, [
      userId,
      firstSourate,
      secondSourate,
      cycleId,
...
const getNotRecitedSourates = async (userId) => {
  ...
  const notRecitedSouratesNumbers = knownSourates.filter(
    (sourate) => !recitedSourates.includes(sourate)
  );
